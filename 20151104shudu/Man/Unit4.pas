...
type
  ...
  TRect = record
    num: TNumRect;//注意TRect的cells是二维数组
    used: TDigit;
    notuse: TDigit; //待用数字——未填入该列的数字
    notuse0: TDigit; //待用数字——不处理占位0
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // 数字已经在小矩阵被使用
              if Rows[r].notuse[index] = pRectTmp.num[ri1][ci1]^ then
              begin
                bExist := True;
                Break;
              end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // 数字已经在小矩阵被使用
              if currNum = pRectTmp.num[ri1][ci1]^ then
              begin
                bExist := True;
                Break;
              end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Rects[ro][co].num[ri][ci] := @Grid[r][c];
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        //数字已被使用，则应该从未使用列表中去掉
        Rows[r].used[c] := iTmp;
        Rows[r].notuse0[iTmp] := 0;
      end;
    end;
    iTmp := 1;
    for c := 1 to 9 do
      if Rows[r].notuse0[c] <> 0 then
      begin
        Rows[r].notuse[iTmp] := Rows[r].notuse0[c];
        Inc(iTmp);
      end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //数字已被使用，则应该从未使用列表中去掉
        Cols[c].used[r] := iTmp;
        Cols[c].notuse0[iTmp] := 0;
      end;
    end;
    iTmp := 1;
    for r := 1 to 9 do
      if Cols[c].notuse0[r] <> 0 then
      begin
        Cols[c].notuse[iTmp] := Cols[c].notuse0[r];
        Inc(iTmp);
      end;
  end;
